...
The foundations of microeconomics entitle us to our own tastes (and beliefs, to some extent), but only if they admit a consistent internal logic.
...
Even programmers agree: 
    when possible, it is best to eliminate conflict by design. 
%
...
We would like to have independent experiments confirm the values of fundamental constants---but this means we must brace for the possibility that the experiments could have different outcomes. 
% If it is non-trivial to prove 
% If we want there to be non-trivial theorems 
We would like to have theorems stating that two different approaches give the same answer---but if the theorems require non-trivial proof,
    then the representation itself could not have guaranteed that the two views are redundant; without the proof, they may well have been inconsistent. 
}%
