...
\newmaterial{%
Economists, seeking to characterize rational thought, have established  probability as the gold standard for representing belief; by using it, one can express uncertainty in a way that is guaranteed to be self-consistent. 
...
Even most programmers agree: 
    when possible, it is better to eliminate inconsistency by design. 
%
...
it becomes impossible to get conflicting answers,
% granting
and so our systems become immune to inconsistency.
}%
...
we will refer to it principally as the divide between
     \emph{structural} and \emph{observational} information.
...
\newmaterial{%
The concepts on the right implement the ones on the left, filling them in with the details needed to make them concrete.
...
Similarly, it is often possible to chain together theorems without knowing their proofs, or describe operations on databases without access to specific data in the tables. 
...
In the graphical models community, the two types of information are known as \emph{qualitative} and \emph{quantitative}; the former traditionally refers to the graphical structure of the model and corresponding independencies of the model, while the latter refers to the concrete probabilistic information involved. 
