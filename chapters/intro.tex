...
The foundational {principles} of decision theory 
% and microeconomics
...
% }}
entitle us to our own preferences
    \citep{de-gustibus-non-est-disputandum77stigler}
    %  (and beliefs, to some extent), 
    \unskip, and beliefs, to some extent
    % \citep{VnMR44theory-of-games-eb}, %, Savage
    \citep{Savage51}
    % but only if they admit a consistent internal logic.
    but only if they have a certain internal consistency.
...
Economists, 
seeking to characterize rational thought, have established probability%
---a representation that allows for uncertainty but cannot encode conflicting information---%
as the gold standard for representing beliefs \citep{ramsey1926truth,Savage51}.
% by using it, one can express uncertainty in a way that is guaranteed to be self-consistent. 
% Computer scientists, making use of such representations, have capitalized on an opportunity to eliminate inconsistency in artificial agents altogether. 
% Computer scientists, by using such representations to build artificial minds, have capitalized on an opportunity to eliminate inconsistency in artificial agents altogether. 
By using such representations to build artificial minds, computer scientists have capitalized on an opportunity to eliminate inconsistency in artificial agents altogether. 
...
Modern ideals of ``good code'' agree: it is better to eliminate the potential for conflict by design, than to handle inconsistency while the code is running. 
%
...
% And no matter how many parameters we add, the goal is to parameterize a single coherent probability distribution. 
% The goal of a learned statistical model is to distill messy and possibly conflicting data into the parameters of a single coherent probabilistic model. 
Why?
...
% immunizing our systems against
\newmaterial{%
% rendering
this renders 
}%
our systems invulnerable to
% our shared affliction: 
% our own affliction of
inconsistency.
...
% Engineers and computer scientists generally agree that \emph{redundancy}---that is, the presence of multiple independent approaches, tests, and safeguards that all agree---is highly desirable. 
\newmaterial{%
It is generally agreed that having multiple independent approaches that give the same result is a virtue, and can increase confidence.
}%. 
...
But, in a sense, (the potential for) \newmaterial{confirmation} and (the potential for) inconsistency are opposite sides of the same coin. 
...
% If we want there to be non-trivial theorems 
    \commentout{We would like to have theorems stating that two different approaches give the same answer---but the representation itself cannot guarantee that the two views are redundant; without the proof of the theorem, they may well instead have been inconsistent.}
}%
...
it is the 
potential for
disagreement that makes agreement valuable. 
...
%
The remainder of the dissertation is based on three previously published conference papers
...
% \textbf{{A Universal Model.}}
% \textbf{{A Natural and Especially General Model.}}
...
% \textbf{{Algorithms, Logic, and Complexity.}}
...
This principle turns out to not only be the basis of an intuitive visual calculus for deriving important inequalities, but also forms the basis of an inconsistency-tolerant probabilistic logic. 
...
Some of these patterns have been important guiding principles;
    others surprised us by their recurrence. 
...
The same conceptual division plays a fundamental role in probabilistic modeling. 
...
%     % Being equivalent to 
% This property---invariance under obvious ``generalizations''---makes objects such as TMs and PDGs particularly special. 
%
...
might be a fundamental representation. 
