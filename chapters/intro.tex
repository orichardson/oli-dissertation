...
In \cref{chap:QIM},
    based on joint work with Peters and Halpern \citep{qim},
    we develop a concept that we call \emph{qualitative mechanism independence}.
...
PDGs, the representation at the heart of this thesis, enables this kind of thinking in the probabilistic setting.
...
\oldnewmaterial{%
We will see this with hypergraphs and bipartite graphs (\cref{sec:prelim-graphs}), with variables, sets of variables, and random variables (\cref{ssec:basic-vars,sec:var-randomvar}) causal models and randomized causal models (\cref{sec:causal}),
    before we even get to our own novel contributions.
}
...
\unskip, but they also capture many natural ``obvious generalizations'' of PDGs themselves. 
...
Not only are many different ways of specifying a PDG equivalent according to a single semantics, 
    but also there are also several different (seemingly less expressive) semantics for PDGs, which are also equivalent.     
}
