...
% using representations that make inconsistency impossible by deisgn.
%
We would rather have one definition of $X$ and one technique for computing $X$ than several different approaches and implementations
...
\Cref{part:foundations} develops foundational concepts that underly the theory of PDGs presented in the previous chapters. 
...
%
Correspondingly, good representations (and tools for manipulating them)
    strive for two fundemental virtues:
...
\newmaterial{%
Some readers might immediately recognize this division; others may find it sweeping at first glance. 
By the end of the thesis, hopefully we will have a better appreciation for why the analogy is appropriate, and specifically its importance for probabilistic modeling. 
}%    
...
We will see this happen almost immeidately even in the preliminary material: with hypergraphs and bipartite graphs (\cref{sec:prelim-graphs}), with variables, sets of variables, and random variables, (\cref{ssec:basic-vars,sec:var-randomvar}), and with causal models and randomized causal models (\cref{sec:causal}),
    % to say nothing of the novel math developped in this thesis.
...
Then, in \cref{part:algo-logic-complexity}, we will see how these two aspects of PDGs are deeply related.  
