...
A hypergraph and its dual correspond to two different ways of notating a composite process: as a \emph{flowchart} or as a \emph{circuit} (see \citet{kasangian1990duality} for a discussion of this duality in a closely related context).
...
The far left one is a chain: operation 1 (op 1) produes X, which is fed to op 2 which produces Y, and finally fed to op 3 to produce Z.
The second hypergraph from the left involves a split and a merge: op 1 produces A, which is copied and sent to ops 1 and 2, which use it to produce B and C, repectively. Then, op 4 uses the values of B and C to produce D.
% The right one, on the other hand, is 
% Is the right one meaningful?  We 
The middle right hypergraph represents a cyclic process, in which op 1 produces B from A and op 2 produces A from B. 
...
Op 1 produces B from A, but op 2 somehow also produces B. 
Is this meaningful?
...
For representing processes like the three on the left,
...
Focusing specifically on machine learning, ``architecture diagrams'' are circuit representations that have become the standard way to communicate the architecture of a neural network.
...
\newmaterial{%
    Note also that, while the left three qualitative BNs all 
}%
