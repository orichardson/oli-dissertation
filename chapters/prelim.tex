...
Broadly speaking, a \emph{graphical model structure}, or a \emph{qualitative graphical model} is a (directed) (hyper)graph whose vertices $\X$ are variables, and whose (hyper)edges somehow indicate local influences between variables.
Its quantiative analogue, \emph{probabilistic graphical model},
or simply  ``graphical model'', is a
graphical model structure together with 
\emph{quantitative} or \emph{observational} information about these local influences.

The point of a graphical model, at least historically, is to be a compact representation of a probability distribution (by exploiding independencies). 
...
a ``qualitative/structural'' aspect, which describes influences between variables 
    (which, critically, determine the independencies that can be exploited),
and a ``quantitative/observational'' aspect, 
    that annotates those influences with data, thereby providing enough information to 
    flesh out a quantiatitve probability distribuiton. 
...
We now review the basics of the most important classes of graphical models used in practice:
    ...
    what we will call factor graphs, which are based on undirected ones. 
...
This intuition can be further clarified through the qualtiative perspective on BNs. 
...
A BN then induces a probability distribution 
\begin{equation}
    \Pr_{\cal B}(\X) := \prod_{X \in \X} P_X( X \mid \Pa(X) );
        \label{eq:bn-product}
\end{equation}
...
By interpreting an edge $X{-}Y \in E$ as a possibility of (bi-directional) influence by $X$ and $Y$ there are three different (but closely related), it also becomes a qualitative graphical model.
...
\begin{itemize}[nosep]
    ...
    \item (local) For each $X \in \X$, let $\partial_E(X) = \{ Y \in \X : \{Y,X\} \in E \}$ be the set of then $X \CI \X \mid \partial_G(X)$.  
    \item (global) If $(X,Y) \notin E$, then $X \CI Y \mid \X \setminus \{X, Y\}$.  
    \item (factorization) there exists a collection of functions 
        $\{ \phi_{\mat X} : \V(\mat X)\to \mathbb R_{\ge 0} \}_{\mat X \in \mathrm{cliques}(G)}$
        indexed by the cliques of the graph $G$,
        such that
        \[
            \mu(\X) = \prod_{J \in \mathrm{cliques}(G)} \phi_K(\X[J])
        \]
\end{itemize}

In general, conditions are distinct, with (global) $\implies$ (local) $\implies$ (pairwise). 
However, if we restrict to \emph{positive} distributions (i.e., $\forall \omega \in \V\!\X.~\mu(\omega) > 0 $), then all four conditoins are equivalent.
