...
\newmaterial{%
Here on out, we will assume that for each variable $X$, the possible values $\V\!X$ not only comes with a $\sigma$-algebra $\mathcal F_X$ (making it a measurable space), but also a base measure $\lambda_X$. 
In the case of a discrete variable that takes on finitely or countably many values, we assume the counting measure: for $U \subseteq \V\!X$ $\lambda_X(U) = |U|$ is the number of elements in $U$. 
...
In the more general setting, the appropriate analogue is the
