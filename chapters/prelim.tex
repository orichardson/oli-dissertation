...
The \emph{disjoint union} of $A$ and $B$ is the set 
...
$\varphi \implies \psi$ means that $\psi$ holds whenever $\phi$ does, and can be viewed as an abbreviation for $\lnot \varphi \lor \psi$, 
...
%% adding variables.
Our notation for representing and manipulate probability distributions will make heavy use of the notion of a variable, as developped in \cref{ssec:basic-vars}. 
...
This confers us a number of benefits,
...
%
To \emph{condition} a joint distribution $\mu(\mat X)$ on a variable $X \in \mat X$ is to form an indexed set of conditioned distributions $\mu(\mat X \mid X) = \{ \mu \mid X{=}x \}_{x \in \V\!X}$, one for each possible value of $X$. 
% The cpd $\mu(Y|X)$
...
write $p(Y|X)$ for a cpd on $Y$ given $X$. In the finite case, cpds coincide with \emph{stochastic matrices}: matrices whose columns correspond to values of $Y$, rows correspond to values of $X$, and such that the sum of each row equals 1. 
