...
Although one effect of this chapter is to develop standard notation for standard concepts, 
    ...
    % the way it fits together is 
% but it also 

The chapter also serves a second purpose: to motivate
    prime the reader 
    to see the most important themes and design choices 
    that go into 
...
The \emph{preimage} of a set $U \subseteq B$ is the set $\{a \in A : f(a) \in B \}$.
...
\paragraph{Linear Algebra, Cones, and Convexity.}
...
\]
Recall that a linear map is one whre this relationship always holds with equality for all $\mat u$ and $\mat v$. 
...
Convex functions and convex sets are closely related. The set of points that lie above a function (its \emph{epigraph}) is a convex set if and only if $f$ is convex. The level sets of a function $f$ (i.e., $\{ \mat u \in \Rext^I : f(\mat u) \le k \}$ for some $k \in \Rext$) are convex if $f$ is convex. And $\mat u \mapsto \infty \mathbbm 1[\mat u \in A]$ is a convex function iff $A$ is a convex set. 
...
If $\Omega = \Delta \V\!X$, we can include an arbitrary random variable 
    $Z : \V\!\X \to \V Z$ in the set $\X' := \X \cup \{Z \}$ of all variables.
When we do so,  
...
$\bar\mu \in \Delta \V\!\X'$ in which variable $Z$ and the function $Z$ applied to the other variables coincide (i.e., $\bar\mu(Z(\X) = Z)=1$).
...
\paragraph{Dependence and Independence.} 
Suppose that $X$, $Y$, and $Z$ are (random) variables and $\mu(X,Y,Z)$ is a distribution over them.
We say that $X$ and $Y$ are \emph{independent} if $\mu(X,Y) = \mu(X)\mu(Y)$.
Precisely the same is true if $X$ and $Y$ are not variables, but rather events.

We say that ${X}$ is \emph{conditionally independent of ${Y}$ given ${Z}$} 
...
\paragraph{Expectation and Utility.} 
If $\mu$ is a probability over $\Omega$, and $X$ is random variable 
    taking values in a convex set (i.e., $\V \! X \subseteq \Rext^I$ cvx), then the \emph{expectation} of $X$ with respect to $\mu$ is its average value, written
\[
    \Ex_{\omega \sim \mu} \Big[ X(\omega) \Big]
        = \Ex_{\mu}[ X ]  
        := \sum_{\omega \in \Omega} \mu(\omega) X(\omega).
\]
In some cases, we omit the braces to reduce clutter and improve readability.

The gold standard for decision making is expected utility maximization: an agent that has an internal probability $\mu$ and a utility function $U: $
