...
\[
\mathbbm1[A]  := x \mapsto {\singlespacingmath
    \begin{cases}
        1 & \text{ if } x \in A \\
        0 & \text { if } x \notin A
    \end{cases}}
...
% \forjoe{(``the Lebesgue measure'' is standard. See \url{https://e.math.cornell.edu/people/belk/measuretheory/LebesgueMeasure.pdf} or \url{https://en.wikipedia.org/wiki/Lebesgue_measure})}
...
\[
    \mathbb A_H = \Big[ \#\Big\{ a \in A : {
            \singlespacingmath
            \begin{array}{c}\Src a = u,\\ \Tgt a = v \end{array}}    
        \Big\} \Big]_{(u,v) \in N \times N}
    \quad
    \mathbb A_G = \Big[
...
% \forjoe{The ``it would appear'' is intentional, because we'll see a in moment that hypergraphs and bipartite graphs are naturally isomorphic.}
...
A hypergraph and its dual correspond to two different ways of notating a composite process: as a \emph{flowchart} or as a \emph{circuit}
% (see \citet{kasangian1990duality} for a discussion of this duality in a related context).
\citep{kasangian1990duality}.
In a flowchart, nodes represent state and  arcs represent transformations between them.
% \forjoe{This was highlighted in the previous document, but you didn't add a comment, and it looks good to me.}
In a circuit, nodes represent operations, and arcs, thought of as ``wires''  connecting the operations, carry state. 
...
People have also considered other ways of making precise the notion of a cyclic baysian network (aside from multiply together factors for a cyclic network), including unrolling it and looking at fixed points---such as dyanmic Bayesian Networks 
% \citep{dean1989model}\citep[\S6.2]{KF09}, 
\citep[\S6.2]{dean1989model,KF09}, 
and a related cut-parameterized approaches to looking at fixed points \citep{Baier_2022}.
...
\else
    DNs can also be \emph{inconsistent}, an idea that is conceptually similar but far narrower than the notion of inconsistency developed in this dissertation. 
\fi
...
\begin{figure}
    ...
    \caption[Illustration of the information profile $\mat I_\mu$]{%
        An illustration of the information profile $\mat I_\mu$
        for a joint distribution $\mu(X,Y,Z)$ over three variables. 
        Each of the seven areas is a component of the vector $\mat I_\mu$. 
        The unconditional entropy $\H_\mu(X)$ of a varable $X$ is the sum of all components in the circle corresponding to $X$. 
        The conditional entropy $\H_\mu(Y|Z)$
            corresponds to cresscent outside of the circle corresponding to $Z$, but inside of the circle corresponding to $Y$.
        The shading on this diagram 
            is a visual demonstration of the identy $\H(X,Y) = \H(X|Y) + \H(Y|Z) + \I(Y; Z)$.
        }
...
\footnote{%
    ...
    In that geometry, 
    \iffoundations we will see (\cref{chap:relent-soup}) that\fi
    gradient flow of relative entropy with respect to its first (``belief'') argument amounts to multiplicative interpolation of probability measures, while gradient flow of relative entropy with respect to its second (``reality'') argument amounts to additive interpolation. 
}
%
