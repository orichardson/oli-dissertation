...
is the structure of
a PDG.
% \citep{pdg-aaai,pdg-infer,one-true-loss}.
...
% by Richardson and Halpern \cite{} 
% by \citeauthor{pdg-aaai} \cite{pdg-aaai}
% \citet{pdg-aaai}
...
\subsection{The Equivalence Between \SCibility\ \texorpdfstring{\\}{} and Solving a Randomized PSEM}
...
% in terms of $\SDef$, the scoring function used for the qualitative structure of PDGs \cite{pdg-aaai}. 
...
% structure, and
structure.  
This function turns out to have an intimate relationship with the qualitative PDG scoring fucntion $\SDef$, which we use to 
%spencer3
...
While we have seen that $\SDef$ has some nice properties,%
    ...
    \end{itemize}}%
    For instance, it captures BN independencies and the dependencies of \cref{theorem:func}, reduces to maximum entropy for the empty \hgraph, and combines with the quantitative PDG scoring function \cite{pdg-aaai} to capture factor graphs.
    }% end footnote
...
See \cref{sec:qual-info-theory-primer} for definitions and a short review.}---the most important quantities in the field---measure
...
%---conditional entropy $\H_\mu(Y|X)$ and conditional mutual information $\I_\mu(Y;Z|X)$\footnote{See \cref{sec:qual-info-theory-primer} for definitions and a short review.}---are 
...
%oli7: 
% which is $\SDef_{\!\Ar}(\mu)$ is a linear function of $\mu$'s information profile.
%oli9:
...
\commentout{
    ...
    The structural deficiency $\SDef_{\!\Ar}(\mu)$ is
...
% It is easy to verify that $\SDef_{\!\Ar}$
%oli10:
% it is also easy to see that $\SDef_{\!\Ar}$
$\SDef$ also
...
satisfies an analogue of \cref{theorem:strong-mono}: if $\Ar \rightsquigarrow \Ar'$, then $\SDef_{\!\Ar}(\mu) \ge \SDef_{\!\Ar'}(\mu)$.
...
%oli11:
% Note, however, that $\SDef_{\!\Ar}(\mu)$ can be
However $\SDef_{\!\Ar}(\mu)$ can be
%oli11:
...
%oli8:
% $\SDef$, 
% the information profile (via $\SDef$)
information deficiency ($\SDef$),
...
$\SDef$ applied to a transformed \hgraph.
...
\begin{lproof}[Alternate, information-based proof]
    ...
    $\SDef_{\Ar_{G}}(\mu) \le 0$. In the appendix of \cite{pdg-aaai}, it is shown that $\SDef_{\Ar_G}(\mu) \ge 0$ with equality iff $\mu$ satisfies the BN's independencies. 
