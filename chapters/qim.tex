...
%joe17*: I actually prefer the previous version.  I'm suggesting a
%compromise.  I don't think we're that far apart (and don't think that
%the war was over the abstract). Here it is:
%spencer3: Ok! I naturally prefer my version, but this is OK, and I like
% the way you bring causality into the picture.
\begin{abstract}
We define what it means for a joint probability distribution to be 
\emph{compatible}
with a set of independent causal mechanisms,
%spencer3: I don't think this is needed. 
% as suggested by Pearl \cite{},
%oli22: not only do I agree with spencer, I think it also is very problematic to but it there. 
%joe18: I would have put it in, and I certainly don't see anything
%problematic abot doing so, but I don't feel strongly about it 
...
When $\Ar$ represents a qualitative Bayesian network, 
{\scibility} with $\Ar$
...
%joe17: We could cut this sentence if we need the space (which we are
%likely to)
%spencer3: I like this edited version better than the original, but I like cutting it more than both :)
% Compatibility also has a close, but far from obvious,
% relationship with the original scoring-function semantics for
% qualitative PDGs, 
% %spencer3:
% % which allows us to prove, for example, the
% % incompatibility
% % of distributions with certain directed hypergraphs. 
...
Similarly, complexity theorists often regard
    ...
    that takes as input a uniformly random string \citep{??}.
}%
...
%wirte some paragraphs explaining the connection.  Please do this by
%11 AM tomorrow.
%oli21: will likely not make it into the NeurIPS submission
\vfull{
\section{SIM-Equivalence}
