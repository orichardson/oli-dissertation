...
it also lets us use graphs as a visual language for
 ...
 %    language for describing independence.
    %     language for describing independence.
    % in part to enable a compact description of probability distributions
...
%joe18: I agree in general about the story
%oli21: Trying for a replacement. 
% In this paper, we focus on the qualitative aspects of probabilistic dependency graphs (PDGs) \citep{} or, more precisely, the hypergraphs that describe their structure, and show how they can be used to describe far more than just conditional independencies.
...
% independence (of mechanism noise variables) plus functional dependence.
%
...
%Press for my books) follow this convention.  Journals/publishers
...
%joe16: I find it incredibly arrogant an inappropriate that, when I
...
%oli8: I've asked you to explain repeatedly why you don't like this, and you've said only that it's unacceptable. I've explained myself repeatedly why I don't like what you're proposing, and you haven't responded to that that. 
...
%
\commentout{%
    \Scibility\ satisfies a \emph{monotonicity} property (\cref{sec:qual-monotone}) that helps us begin to understand these cyclic models.}%
%
...
%     % stronger predicate on distributions (see \cref{sec:qual-monotone}). 
%     (see \cref{sec:qual-monotone}).
...
\cref{sec:qual-monotone,sec:qim-info} to better understand what \scibility\ means in these cases.
...
\begin{linked}{theorem}{func}
    ...
    $\mu \models \Diamond \Ar$ iff $\mu \models \Diamond \Ar_0$ and $\mu \models X \tto Y$. 
        ...
        if and only if $\mu \models  \Diamond\Ar_0 \sqcup \{X \to Y\}^n$ for all $n > 0$.
    \end{enumerate}
\end{linked}
}
...
\commentout{
    ...
    \end{tikzpicture}$, a condition we will define in \cref{sec:qual-monotone}),
...
% As we discuss in \cref{sec:qual-monotone}, 
\begin{example}
...
%oli18*:
\commentout{
In this section, we spell out the fundemental connection between causality and \scibility. In brief: $\mu \models \Diamond \Ar$ if and only if it is possible for $\mu$ to arise from a causal model with dependency structure $\Ar$ (\cref{ssec:cm-arise}); furthermore, a witness of \scibility\ can be naturally converted to such a causal model. To make this precise, we must review some standard definitions from causality. }
...
% Let $\mathrm{do}(\mat Y{\gets}\mat y) := \cap_{Y \in \mat Y} (\hat U_Y = \lambda z.~\mat y[Y])$ denote the event in which each variable $\hat U_Y$ takes on the constant function that outputs $\mat y[Y]$ for all inputs. 
% Let $\mathrm{do}(\mat Y{\gets}\mat y) := \cap_{Y \in \mat Y} ( \forall z.~f_Y(U_Y, z) = \lambda z.~\mat y[Y])$ denote the event in which each variable $\hat U_Y$ takes on the constant function that outputs $\mat y[Y]$ for all inputs. 
% \[
% % \Pr_\nu \Big(
...
%oli6: these are just dependencies, so this might clarify somehow
...
%is to you.  But let me assure you that you're not typical, and it
%won't be intuitive to many readers (who will also find your notation
...
Perhaps surprisingly, \Scibility\ with $\Ar_M$ and $\Ar_{\cal M}$
    ...
    (i.e., $\mu \models \Diamond \Ar_M$ iff $\mu \models \Diamond \Ar_{\mathcal M}$).
But that does not make the two structures interchangeable,
...
%joe3: I would cut the next paragraph.  I don't really understand it.
...
%joe8: cut; it's a bad idea to talk about "wishful" simplifying assumptions"
...
%generalization (which will no doubt confuse soe readers).
...
%oli20: replacement:
%joe16*: I see no English reading where allowing an equation to depend
...
%oli21*: I'm not sure how exactly, but I suspect there's some misunderstanding between us. Let me try to be really precise.  Fix a PSEM M. In that PSEM, each equation is a (deterministic) function of the variables. It is indeed a generalization to allow those not to be deterministic functions, but randomized ones, that can flip coins.  This is because we've swapped the notion of a function out for the notion of a stochastic map, which is more general. Yet, because stochastic maps are equivalent to deterministic functions that take an additional argument of additional random bits, this "generalization" of a PSEM does not buy any expressive power. 
...
%already asking the point by saying they have ore expressive power in
...
%joe14: the fact that randomized PSEMs capture the information
...
%joe8*: I actually don't see why this is an expansion.  In any case,
...
% In each case,
In both cases,
the high-level result is the same: $\mu \models \Diamond \Ar$ iff there is a 
%joe8:
...
%          If $\Ar = \Ar_{\cal M}$ for some PSEM $\cal M$, then $\mu \models \Diamond \Ar$ iff $\mu$ can arise from a (possibly different) randomized PSEM of the same dependency structure $\Ar$. 
%      \end{prop}
%     }%
...
(implicit in the assertion $\mu \models \Diamond \Ar$) and the causal model $\mathcal M$ (implicit in the statement that $\mu$ can arise from some causal model) have been obfuscated behind existential quantifiers. 
...
\begin{itemize}[wide]
    \item 
    When $\Ar = \Ar_G$ for some is a directed acyclic graph $G$ (i.e., a qualitative BN), then a witness to $\mu \models \Diamond \Ar$ is
    ...
    %oli6: already mentioned
        ...
        % strongly gave me the impression some version of this fact is is common knowledge. 
        %joe2: It certainly isn't common knowledge! I don't know it.
        %oli3: first part is now no longer necessary because it's in the right place
        ...
        % From this perspective, \cref{theorem:bns} is a form of an equivalence  well-known in the causality community:
...
%rewrite this material significantly.  
...
%lots of causal models with endogenous variables \X and exogenous
...
%other notions of interest (like the semantics for qualitative models_)
%You haven't said anything about the case of dags.   I would start
%with that.  You're missing the chance to tell a nice story here.  I
...
%oli3: reclaiming some extra words here
...
\commentout{
In other words, if you convert a distribution $\bar\mu$ witnessing $\mu \models  \Diamond \Ar$ to a causal model $\mathcal M \in \PSEMs(\nu)$, then 
% (a)
...
%joe8
% a
%oli19: this "a" changes the meaning; I meant that the randomized PSEMs (which have dependency structure A) can give rise to \bar \mu, not that their dependency structure can. 
...
% over contexts $\V(\U)$
\begin{equation}
    % oli14: perhaps worth pointing out  that this depends on
...
%oli19: We can get rid of the X if we only intervene on one  variable at a time, but that weakens the theorem for no real reason. This means the value of the variable X in the joint setting \mat x = (x_1, x_2, x_3, ... ).  One of those components corresponds to the variable $X$. This notation needs to be defined, but it needs to be defined earlier, because it's also used in the construction. 
...
%and absolutely does not give intuition.
...
%know what nonstandard probabilities are.  It feels like you're doing a
...
% For the sake of simplicity and concordance with the causality literature, take $\PSEMsA(\bar\mu)$ to consist only of ordinary (i.e., not generalized) PSEMs, effectively restricting scope of the theorem statement to \subpartl\ \hgraph s $\Ar$.
...
% except I actually need a few more premises to be perfectly precise. 
% If $\nu \in \Wits(\mu,\Ar)$ and
...
% Suppose $\nu(\X,\U) \in \Wits(\mu,\Ar)$
%oli15: 
% Suppose $\bar\mu(\X, \U)$ is a \scibility\ witness to $\mu \models \Diamond \Ar$,
Suppose that
%oli21: getting rid of a QIM and otherwise shortening
% $\bar\mu(\X, \U_{\Ar})$ is a \scibility\ witness to $\mu \models \Diamond \Ar$,
...
%make all along.  (Notice all my "I have no clue what you're doing"
%comments.)  
...
%oli18:
% are typically (and for good reason!) regarded as fundamentally different. 
are usually thought of as fundamentally different. 
(Indeed, intervening on $\mat X{=}\mat x$ remains fundamentally different from conditioning on $\mat X{=}\mat x$.)
Nevertheless, 
    % both facts are true, 
...
% but we have shown how to encode the capacity for interventions latent in a causal model directly into a distribution (a witness). 
%joe8
...
\commentout{%
    ...
    %This fact has allowed us to deepen the connection between causality
...
%joe3*: A witness is a distribution.  At a minimum, you have to
% explain how you're viewing the distribution as a causal model, and
% why it should be acrylic in this case.  I'm lost here.
%oli3*: this is meant as a preview; I'd like to prime the readers for
% the technical details they'll see in the next section. I'll add some
% extra verbiage to make the intended purpose of this section clear.
%
%
in which the equation for $X$ depends on $\Pa_G(X)$ and
%joe3*: In what sense does it "ultimately give rise to \mu$?  This has
...
%oli6: already mentioned
    ...
    \begin{prop}
        ...
        $\mu \models \Diamond \Ar_G$, and $G$ is minimal 
            (i.e.,$\mu \not\models  \Diamond \Ar_{G'}$ for all $G' \subsetneq G$),
...
\end{figure}
%oli20: replacement
We now review a few critical information-theoretic concepts and their relationships to
    (in)dependence
...
% For three variables, the components of this vector are illustrated in \cref{fig:info-diag-a}.
For three variables, the components of this vector were illustrated in \cref{fig:info-diag-prelim}; 
for the convenience, 
    % we give another illustration of it in \cref{fig:info-diag-a}.
    we give another illustration of it here \cref{fig:info-diag-a},
    colored to illustrate an identity which will be important momentarily.
% \forjoe{
%     looking at this diagram is important at this point in the text. Also, this one has a slightly different annotation.  The previous one had more diversity of different concepts; this one gives an intuitive visual proof of a property we need in the next two pages.}
%
%oli15: moved above:
...
Still, the quantity $\SDef_{\!\Ar}(\mu)$ does not determine whether or not $\mu \models \Diamond \Ar$. 
...
% In \cref{sec:qim-bns} we saw how, for \hgraph s that come from directed graphs, this means $\mu$ must satisfy the independencies of a Bayesian Network, and that additional \arc s can impose functional dependencies---but these results apply only to special classes of \hgraph s. 
% We saw in \cref{sec:qim-bns} that $\mu \models  \Diamond\Ar$ implies dependencies for \hgraph s of  
%oli15: compressing, sharpening
% In \cref{sec:qim-bns} we saw how, for \hgraph s that come from directed graphs, $\mu\models  \Diamond \Ar$ implies $\mu$ must satisfy the independencies of a Bayesian Network, and that additional \arc s impose functional dependencies---yet these results apply only to very special classes of \hgraph s. 
...
\commentout{%
    In \cref{sec:qual-monotone} we saw how we can weaken \scibility\ relationships, but information is lost in the process, and the resulting \scibility\ statement often no easier to interperet.
}%
...
% tensored with
%oli20:
% plus a seemingly irrelevant bit $X_1$ written in the sky.
...
%case, it depends on your intuition.  In general, it's a bad idea to
...
%oli6: moved below (with modification):
    ...
    % This also shows that the distribution $P$ does not so fundamentally require a 3-way interaction between the three variables, even if it seems that way because of how it is constructed.
%joe5*: I'm totally lost.  In what way P fundamentally require
%anything.  More importantly, this is far removed  from anything even
%remotely related to what I see as the story of the paper.
...
%oli6: compressing; most of this has been said.
    % This shows that, despite appearances, $P$ does not fundamentally require a 3-way interaction, unlike the distribution $\muxor$ on which $P$ is based.
    %oli15:
...
%oli21: accommodating wrapfig: part 2
% \end{example}
\hfill$\triangle$
...
%oli3:
% For each $a \in \Ar$, fix a positive real number $\lambda_a > 0$. Also, fix $\lambda_0 > 0$. With these, we can define a new scoring function parameterized by
...
%oli14:
%oli20: redoing this
% It may look complicated, but the terms on the first line are a measure discrepancy between $\nu$ and condition (b) for being a \scibility witness (\cref{defn:qim}), those on the second line measure discrepancy with condition (c), and the constraint under the infimum ensures we only search over distributions satisfying (a). 
...
% The first is a measurement of discrepancy between $\mu$ and the marginal $\nu(\X)$, and the third is the non-zero iff 
...
%oli14:
% However $\SIMInc$ is not exactly a linear function of its argument's information profile; because of the infimum, it encodes an optimization problem over such functions.
%oli20: cutting, as agreed
\commentout{
Although it is built from a sum of entropies, $\SIMInc_{\Ar}$ is not a linear function of its argument's information profile; rather, because of the infimum, it is an optimization problem over the information profiles of extended distributions.
...
\commentout{
    ...
    weight that intuitively represents the  modeler's degree of
    ...
    % expediency and to accommodate a story you're very fond of but is
    % slightly wrong. 
    }
}%
%
...
$\SDef$---not for the \hgraph\ $\Ar$, but rather for a transformed one $\Ar^\dagger$ that models the noise variables explicitly. 
...
\commentout{
    ...
    % one that outputs $U_a$ without taking inputs, and another
...
% The lower bound generalizes \cref{theorem:sdef-le0} to cases where $\mu \not\models  \Diamond \Ar$,  while the tight upper bound shows that $\SIMInc_{\!\Ar}$ can be defined in terms of $\SDef_{\!\Ar^\dagger}$.
...
\begin{linked}{theorem}{siminc-idef-bounds}
    ...
    \[% ... and layout option 2
        ...
        = \min \Big\{\ \SDef_{\!\Ar^\dagger}(\nu) : 
            {\singlespacingmath\begin{array}{c}
            \nu \in  \Delta\V(\X,
            %oli10:
                % \hat \U_\Ar^\X
                \U
...
\begin{conj}
    ...
    Suppose $\mu \models  \Diamond \Ar$, $\mu' \models  \Diamond \Ar'$, and $\alpha \in [0,1]$.
...
\commentout{
    ...
    Another obvious question is the decision procedure for deciding whether or not $\mu \models \Diamond \Ar$.
}%
...
\begin{lproof}
    \label{proof:bns}
    % Suppose $\mu \models  \Diamond \Ar_G$.
...
\begin{lproof}[Alternate, information-based proof]
    \textbf{($\implies$).}
    Let $G$ be a dag.     
    If $\mu \models  \Diamond \Ar_{G}$, then by \cref{theorem:sdef-le0}, 
    $\SDef_{\Ar_{G}}(\mu) \le 0$.
...
Suppose that $\mu \models  \Diamond \Ar$ and $\mu \models X \tto Y$.
...
Thus $\nu$ (or, more precisely, an isomorphic ``extension'' of it to additional trivial variables) is a witness of
 % $\mu \models \Diamond \Ar' \sqcup \{ \ed1XY, \ed2XY\}$. 
...
Suppose that $\mu \models  \Diamond \ArXY n$ for all $n \ge 0$. 
% Observe that 
...
By monotonicity, this means $\mu \models  \Diamond \Ar$ and also that $\mu \models\begin{tikzpicture}[center base]
    \node[dpad0] (X) {$X$};
...
As mentioned above, we also know that $\mu \models  \Diamond \Ar$, 
and thus $\mu \models \Diamond \Ar ~\land~ X {\tto} Y$ as promised.
\end{lproof}

% \newpage
\subsection{Causality Results of \texorpdfstring{\cref{sec:causal}}{Section \ref*{sec:causal}}}
%oli15*: proof
...
\begin{lproof}\label{proof:sc-graph-arise}
    ...
    \end{itemize}
    %
    Under the assumption that $\mu \models  \Diamond \Ar_G$, we have now shown that there exists a randomized causal model $\cal M$ from which $\mu$ can arise, with the property that 
...
\commentout{
    ...
    We claim that this $\nu$ is a witness to $\mu \models  \Diamond \Ar_G$. 
    ...
    Thus $\nu$ satisfies all conditions (a-c) for a \scibility\ witness, and hence $\mu \models  \Diamond \Ar_G$. 
\end{lproof}
...
\begin{lproof}\label{proof:gen-sim-compat-means-arise}
    \textbf{($\implies$).~} Suppose $\mu \models \Diamond \Ar$, meaning there exists a witness
    ...
    We claim that this $\nu$ is also a witness that $\mu \models \Diamond \Ar$.
...
\begin{lproof}\label{proof:condition-intervention-alignment}
    ...
    \[ 
        ...
        \nu(\doXx) 
    \]
    is defined. By assumption, $\mathcal M \in \PSEMs(\nu)$ and $\nu$ is a witness to $\mu \models \Diamond \Ar$. Thus, by \cref{prop:witness-model-properties}, we know that $\nu \in \SD{\mathcal M}$. 
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            &= P(\U_{\mat X}) P(\U_{\mat Z}) 
                &\left[~\parbox{6cm}{\singlespacingmath since $\doXx$ depends only on $\U_{\mat X}$, while $\U_{\mat X}$ and $\U_{\mat Z}$ are independent in $\nu$ (by the witness condition).}~\right]
                \\
            &= P(\U_{\mat X}, \U_{\mat Z})
    ...
    \[
        ...
        {\singlespacingplusmath \begin{array}{rl}
...
%     1.
%     Suppose $\mu \models  \Diamond \Ar_G$, $\bar\mu(\U,\X)$ is a witness to this, and $\mathcal M \in \PSEMs_{\Ar_G \!}(\bar\mu)$. 
% 
%     \textbf{($\implies$).}
...
% \subsection{Monotoncity Results of \cref*{sec:qual-monotone}}
% \recall{theorem:mrf-bn-monotone-impossible}



\subsection{Information Theoretic Results of \texorpdfstring{\Cref{sec:qim-info}}{Section~\ref*{sec:qim-info}}}
...
The following fact, known as the \emph{chain rule for mutual information} will also be critical for our calculations:
    ...
    \begin{align*}
        ...
        &= \I(X;  Y_{k+1} \mid \mat Y_{1:k}) + \I(\mat Y_{1:k} ;  Y_{k+1})
            & \big[\text{ by MI chain rule \eqref{eq:chain-rule-mi} } \big] \\
        &= \left\{
        {\singlespacingplusmath \begin{array}{cc}
...
\begin{lproof}
    ...
    Suppose that $\mu \models \Diamond \Ar$, meaning that there is a witness $\nu(\X, \U)$ that extends $\mu$, and has properties (a-c) of \cref{defn:qim}. 
    ...
    \begin{align*}
        ...
        &= \I_\nu(\X ; \U) 
            &
            {\singlespacingmath \hspace{-1.5cm}
...
%         If $\mu \models  \Diamond \Ar_G$, then 
...
\begin{prop}
    $\mu \models \Diamond \Ar \sqcup \{ \ed1{}X, \ed2{}X \}$ if and only if $\mu \models \Diamond \Ar$ and
    %  $\H_\mu(X) = 0$.
...
In other words, it is still not clear how to solve the decision problem: given $\mu$ and $\Ar$, determine whether or not $\mu \models  \Diamond \Ar$.
...
\begin{conj}
    If $\mu_0 \models \Diamond \Ar$, and $\mu'$ lies on the path $\mu(t)$ of
    gradient flow minimizing $\SDef_{\Ar}(\mu')$, starting at $\mu(0) = \mu_0$, then $\mu' \models  \Diamond \Ar$.
\end{conj}
...
\begin{conj}
    % If $\mu \models \Diamond \Ar$
    The distribution (s?)
    $\hat \mu := \arg\min_{\mu : \mu \models  \Diamond \Ar} \kldiv{\mu}{\hat\mu}$
...
\begin{defn}
    ...
    We say a witness $\nu(\U,\X)$ to $\mu \models \Diamond \Ar$ is \emph{even}, iff
...
%oli11:
and finite when $\mu \models  \Diamond \Ar$. 
