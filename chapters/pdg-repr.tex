...
encode all of the relevant conditional marginals. More precisely, fix a
    ...
    %     Consider a simple example in which encodes a distribution $p$ over the set $W$. In this case, there is only one edge, the expectation is over
...
\item 
% separately specify, for each edge $\ed aXY$, the value $\mat w_Y$ (of $Y$ in world $\mat w$) given the value $\mat w_X$, in expectation.
separately specify, for each edge $\ed aXY$, the value $\omega[Y]$ (of $Y$ in world $\omega$) given the value $\omega[X]$, in expectation.
