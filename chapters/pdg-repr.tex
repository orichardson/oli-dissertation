...
\begin{enumerate}[wide, label={\textbf{PDG Variant \arabic{*}:}}]
    ...
    \item 
	\textbf{``True'' PDGs. }
	As mentioned in the definition, the structure $(\N, \Ar)$ of a PDG can be taken to directly be an (ordinary) directed graph rather than the (seemingly more expressive) directed \emph{hyper}graph. 
    ...
    One might want to also consider a \emph{parametric} variant of a PDG,
		 in which each cpd is not fixed, but dependent on some hyper\-parameters. 
