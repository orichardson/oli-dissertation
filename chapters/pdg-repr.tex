...
$F$ and $G$ (respectively taking values $\{f, \lnot f\}$ and
$\{ g, \lnot g\}$),
...
%
\vfull{\unskip\footnote{In the factor graph literature,
          ...
          gives an alternate expansion of ``PDG'' 
...
\item[$\alpha$] \notation{$:\Ar \to [0,1]$}
associates to each edge $\ed aXY$ a non-negative number $\alpha_L$ which,
...
as $\dg M = (\Gr,\mathbb P, \alpha,\beta)$, where $\Gr = (\N,\Ar,\V)$, and
...
to $Y$,  $x \in 
\V(X)$,  and $y \in \V(Y)$, we have that $\mu(Y \!=\! y \mid X{=} x) = \p_a(T|s)$.
\notation{Formally,		
    \[ \SD[\Big]{\dg M} = \!\left\{\mu \!\in\! \Delta \V (\dg M) \middle|\!
        \begin{array}{l}
...
\vfull{
	The \emph{inconsistency of PDG $\dg M = 
        (\N, \Ar, \mathbb P, \balpha, \bbeta)
        $}, denoted $\Inc(\dg M)$, is the minimum possible incompatibility of $\dg M$ with any distribution $\mu$,  
...
\vfull{
    ...
    distribution $\mu$. For each edge $L = (X, Y, \ell) \in \Ar$ and $x \in
    ...
    probability $\mu_X(x)$. We do this for every edge $L \in \Ar$, summing the
    cost.
    ...
    distributed according to $\mu$. For each edge $L = (X,Y, \ell) \in \Ar^\dg
...
$\Inc_{\dg M}(\mu)$ and $\IDef_{\dg M}(\mu)$ give us two measures
...
\begin{equation}
  	  \bbr{\dg M}_\gamma(\mu)
	 := \Inc_{\dg M}(\mu) + \gamma \IDef_{\dg M}(\mu)  \label{eqn:full-score}
\end{equation}
...
If $\dg N = (\Gr, \mathbb P)$ is an unweighted PDG, define   
...
\bigg[\,
    \color{gray}\overbrace{\color{black}
      \!\beta_L \log \frac{1}{\p_a(y^{\mat w} |x^{\mat w})}
...
\item They allow for a clean separation between quantitiatve information (the
	...
	terms $\Inc$ and $\IDef_{}$ in our scoring function.
...
information is weighted by $\alpha_a$.
  \begin{align*}
	\bbr{\dg M}_\gamma(\mu) &:= \Inc_{\dg M}( \mu) + \gamma \IDef_{\dg M}(\mu) \\
		&= \left[\sum\alle \beta_L \Ex_{x\sim \mu_X}\kldiv[\Big]{ \mu(Y | X {=} x) }{\p_a(x) } \right]  + \gamma \left[\sum\alle \alpha_L \H_\mu(Y\mid X) ~-\H(\mu)\right]\\
		&= \sum\alle 
			\Ex_{x \sim \mu_{\!_X}}  \left[ \beta_L\; \kldiv[\Big]{ \mu(Y \mid x) }{\p_a(Y \mid x) } + \gamma \; \alpha_L \H(Y \mid X{=} x) \right]  - \gamma \H(\mu) \\ 
		&= \sum\alle 
			\Ex_{x \sim \mu_{\!_X}}  \left[ \beta_L\; \left(\sum_{y \in \V(Y)} \mu(y \mid x) \log\frac{\mu(y\mid x)}{\p_a(y\mid x)}\right) + \alpha_L\gamma \; \left(\sum_{y \in \V(Y)} \mu(y\mid x) \log \frac{1}{\mu(y\mid x)} \right) \right]  - \gamma  \H(\mu) \\ 
		&= \sum\alle 
			\Ex_{x \sim \mu_{\!_X}}  \left[ \sum_{y \in \V(Y)} \mu(y \mid x) \left(  \beta_L\; \log\frac{\mu(y\mid x)}{\p_a(y\mid x)} + \alpha_L \gamma \; \log \frac{1}{\mu(y\mid x)} \right) \right]  - \gamma  \H(\mu) \\
		&= \sum\alle 
			\Ex_{x \sim \mu_{\!_X}}  \left[ \Ex_{y \sim \mu(Y \mid X=x)} \left(  \beta_L\; \log\frac{\mu(y\mid x)}{\p_a(y\mid x)} + \alpha_L \gamma \; \log \frac{1}{\mu(y\mid x)} \right) \right]  - \gamma \sum_{\mat w \in \V(\dg M)} \mu(\mat w) \log \frac{1}{\mu(\mat w)} \\  
		&= \sum\alle 
			\Ex_{x,y \sim \mu_{\!_{XY}}}  \left[ \beta_L\; \log\frac{\mu(y\mid x)}{\p_a(y\mid x)} + \alpha_L\gamma \; \log \frac{1}{\mu(y\mid x)}  \right]  - \gamma  \Ex_{\mat w \sim \mu} \left[ \log \frac{1}{\mu(\mat w)}\right] \\
		&= \Ex_{\mat w \sim \mu} \Bigg\{   \sum_{ X \xrightarrow{\!\!L} Y  } \left[
			\beta_L \log \frac{1}{\p_a(y\mid x)}   - \beta_L  \log \frac{1}{\mu(y \mid x)}+ \alpha_L\gamma \log \frac{1}{\mu(y \mid x)} \right]\Bigg\}  -  \gamma  \Ex_{\mat w \sim \mu} \left[\log \frac{1}{\mu(\mat w)}\right] \\
		&=  \Ex_{\mat w \sim \mu} \Bigg\{ \sum_{ X \xrightarrow{\!\!L} Y  } \left[
...
\bbr{M}_\gamma(\mu) 
	...
	&= \Ex_{\mat w \sim \mu} \Bigg\{   \sum_{ X \xrightarrow{\!\!L} Y  } \left[ \gamma {\alpha_L} \log \frac{1}{\p_a(y\mid x)} + 
		(\beta_L - {\alpha_L} \gamma) \log \frac{1}{\p_a(y\mid x)} - (\beta_L - {\alpha_L} \gamma) \log \frac{1}{\mu(y \mid x)} \right] - \gamma \log \frac{1}{\mu(\mat w)} \Bigg\}  \\
	&= \Ex_{\mat w \sim \mu} \Bigg\{   \sum_{ X \xrightarrow{\!\!L} Y  } \left[ \gamma {\alpha_L} \log \frac{1}{\p_a(y\mid x)} + 
...
Since $\bbr{\dg M}_\gamma = \Inc_{\dg M} + \gamma \IDef_{\dg M}$,
...
both $\Inc$ and $\IDef_{}$ are bounded below, it must be the case that  
\[
...
\[ \bbr{\dg M}_\gamma(\mu_i) = \Inc(\mu_i) + \gamma\IDef_{\dg M}(\mu_i)
> \Inc(\nu_2)  
...
that $\IDef_{\dg M}(\nu_1) =\IDef_{\dg M}(\nu_2)$.  
    ...
    $\gamma > 0$, 
    \[ \bbr{\dg M}_\gamma(\mu_{i}) = \Inc(\mu_{i}) + \gamma\IDef_{\dg M}(\mu_{i}) > \Inc(\nu_3) 
...
We have now shown that that $\IDef_{\PDGof{\mathcal B, \beta}}$ and $\Inc$ are 
...
% 	$\bbr{{\dg M}_{\Psi,\gamma}}_{\gamma}^*$.
% }
\recall{theorem:fg-is-pdg}
\begin{lproof}\label{proof:fg-is-pdg}
