...
\begin{enumerate}[wide]
    \item As mentioned in the definition, the structure $(\N, \Ar)$ of a PDG can be taken to directly be a directed graph rather than the (seemingly more expressive) directed hypergraph. 
	...
	This approach is an instance of the approach to probabilistic modeling that emphasizes variables, detailed in \cref{sec:var-randomvar,ssec:fin-prob}
    ...
    \item Going even further, the restriction to PDGs in which $(\N, \Ar)$ is not a multi-graph, but rather an ordinary directed graph, is not a restriction at all. It is not hard to see that a multi-graph could be simulated in that formalism with the following widget:
    \[
        \begin{tikzpicture}[center base]
...
Notice how this parallels overhead of erroneous quantitative beliefs discussed above.
If \ref{item:globalinfo} $>$ \ref{item:localinfo}, 
...
which is precisely what the literature tells us to do, at least, to break ties between distributions consistent with our observations in the absence of no structural or causal information \cite{jaynes1957information}. 
...
Since $\SDef_{\!\Ar}$ is a linear combination of (conditional) entropies,
it is also a linear function of $\mu$'s information profile vector.
Technically, this means $\SDef_{\!\Ar}$ is a dual vector in information profile space, and thus can be illustrated with the same diagram as used to show information profiles.
A substantial number of small examples have been visualized this way in \cref{fig:sdef-example-panel}.
We now make a few observations about that figure. 

\begin{enumerate}
\item
Observe that, in the definition of $\SDef$, each arc $X\to Y$ is compiled to a ``cost'' $H(Y|X)$ for uncertainty in $Y$ given $X$.
...
\item
 For more complex structures, structural information deficiency $\SDef$ 
...
%
For several reasons---%
	most importantly, to simplify the math, 
	and to bring out relationships with other graphical models and the thermodynamics literature%
...
The three semantics given in the previous section are the ones presented in the original PDG paper \cite{pdg-aaai}.
...
graphs. PDGs are strictly more general than BNs, and can emulate factor graphs for a particular value of $\gamma$. 
More precisely, the distribution specified by a BN $\dg B$
is the unique one that minimizes both $\OInc_{\dg B}$ and $\SDef_{\dg B}$ (and
