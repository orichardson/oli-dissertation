...
time to reflect, we can represent her (conflicted) knowledge state as the sum
of the two graphs, depicted graphically on the right of \Cref{fig:grok-combine}.
\forjoe{ I still believe the text above is accurate. It  }
...
Indeed, the ``appropriate thing'' to do seems to depend on context, and so we urge the modeler to think about which qualitative mechanisms ought to be merged together, after independent combination. 
We will return to this in \cref{ex:overdet}.
The
...
analogue of this problem is even bigger 
    ...
    $\balpha$ and $\bbeta$ are essentially ``fused together'' in these models, as we will see in \cref{sec:factor-graphs}.
...
Because of the inherent asymmetry, 
    ...
    there is no inherrent reason to prefer
    a symmetrical formalims (like the one with $\zogamma$). 
...
if for every node $X$ with multiple parents, the entropy of conditional probabiliy $\mathbb P_X(X \mid \Pa(X){=}\mat z) \in \Delta \V \! X$ does not depend on $\mat z$. 
To see why this would help, it is helpful to see
    an example of a BN whose distribution does not encode maximum entropy. 
...
\begin{example}\label{ex:counterexample}
    ...
    As for the probability tables, suppose that both $A$ and $B$ get uniform unconditional probabilities ($\nf12$ apiece), and set $C$'s cpt to be
	\[
        \p_C(C \mid A,B) = 
    ...
    Thus, the BN's distribution has entropy $H(\mu_{\text{BN}}) = 2 + k/2$ (one for each of the independent bits, and an expected k/2 bits from getting the uniform distribution on $C$ half the time).
...
%
\Cref{theorem:bns-are-pdgs} is our first illustration of this. 
}
