...
\begin{enumerate}[wide, label={\textbf{PDG Variant \arabic{*}:}}]
    ...
    the structure $(\N, \Ar)$ of a PDG can be taken to directly be an (ordinary) directed graph rather than the (seemingly more expressive) directed \emph{hyper}graph. 
...
In addition, when $\bbeta = \gamma \balpha$,
...
Alternatively, $\gamma$ can be thought of as one final degree of confidence, in the same scale as the $\beta$'s, in the overall qualitative picture (relative to the other pieces of quantitative information).

For all of these reasons, and also to simplify the math, we have chosen to use the asymmetric variant in \eqref{eqn:scoring-fn} that uses the parameter $\gamma \in [0,\infty]$ to control the strength of $\SDef$, rather than the symmetric trade-off parameter $\zogamma \in [0,1]$.
}
