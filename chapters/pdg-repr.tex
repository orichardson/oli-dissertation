...
% For the qualitative information, however, independent combination might not 
Independently combining the qualitative information, however, could have unintended effects if the qualitative information has any overlap. 
...
However, rather than viewing them as two different mechanisms, one might well have wanted something else: two independent accounts that there is a \emph{single} such causal mechanism. 
% This inability to  with factor graphs, which we discuss in \cref{ex:overdet}. 
The latter is not captured by PDG union; indeed, we have not found an algorithm we endorse to implement that intuition, and so we urge the modeler to think carefully about the appropriate causal picture in the combined PDG.
% This inability to  with factor graphs, which we discuss in \cref{ex:overdet}. 
We mention that there is an even bigger problem in combining factor graphs, because the parameters $\balpha$ and $\bbeta$ are essentially ``fused together'', as we will see in \cref{sec:factor-graphs},
and thus this leads to an even bigger problem in combining factor graphs; we will explore this in \cref{ex:overdet}. 

%TODO: Convex combination of 
It is worth mentioning that \emph{convex combinations} of qualitative parameters $\balpha$ do work as one might hope. If you believe that there's a 50\% chance that the causal graph is $G_1$, and a 50\% chance that the causal graph is $G_2$, there is no problem with taking $\balpha$ to be $.5 \balpha_1 + .5 \balpha_2$, where $\balpha_1$ and $\balpha_2$ represent the causal graphs $G_1$ and $G_2$, respectively.  For instance, if $G_1$ and $G_2$ represent the same independencies, then $\balpha$ will encode those shared independencies. 

Still, there are some cases where independent combination is appropriate even for causal information; we now proceed with the formal details.
}
...
% Indeed, if
If in place of the pair $(\N, \V)$, a PDG is specified directly using a set $\X$ of variables, then \cref{defn:v-compat} is unnecessary, as its sole purpose is to ensure that we do not accidentally identify variables in different PDGs that do not take on the same values. 
