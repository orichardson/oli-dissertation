...
Although it is possible to formalize PDGS with hyperarcs directly,
    we opt for a different approach here, in which PDGs have only regular arcs,
and hyperarcs are captured using a simple construction
...
%       \emph{pseudomarginals} \cite{wainwright2008graphical}.
%     Syntactially, a PDG is therefore 
...
\begin{description}[topsep=0pt]%
	...
	\item[$\Ar$] \notation{$\subseteq \N \times \N \times \mathit{Label}$}%
		is a set of labeled arcs $\{ \ed aXY \}$, each with a source 
		$S$ and target $T$ in $\N$;
	\item[$\V$] \notation{$: \N \to \Set$}%
...
\item[$\bbeta$] 
    \notation{$:\Ar \to \Rext$}
associates to each arc $a$ a (possibly infinite) real number $\beta_a$,
the modeler's 
...
the arc from $C \times T$ to $C$ gives probability 1 to $c$;
similarly, the cpd for the arc from $ C \times T$ to $T$ gives probability 1 to $t$.
\end{constr}
...
We now review a some important alternative ways of defining a PDG.
...
\subsection{PDG Superposition}

As illustrated \cref{ex:grok-union}, a significant advantage of PDGs is their modularity:
...
% 	(because more arcs should count double), 
%     but we don't want to duplicate variables.

% If we're being extra careful,
To be more precise, depending on the variant
...
We now develop the full PDG semantics. 
...
Given $\mu \in \Delta \V\!\X$,

returns a real-valued score,
% indicating the degree of \emph{incompatibility} between $\mu$ and $\dg M$.
indiating how well (or, rather, how poorly) $\mu$ matches the information in $\dg M$. 
...
These two aspects of the scoring function corresond 
...
\begin{align*}
    \bbr{\dg M}_\gamma&(\mu) 
        ...
        &= \scalebox{0.85}{$\displaystyle\frac{1}{1-\zogamma}$} \Big(\, (1-\zogamma) \OInc_{\dg M}(\mu) + \zogamma \, \SDef_{\dg M}(\mu)\, \Big) \\[-0.1ex]
        &= \Ex\nolimits_{\mu}\bigg[
            \,
            \sum_{\ed aST \mathrlap{\, \in \Ar}} \subafalse
            \log \frac
...
particular, for an arc $\ed aXY$ and $x \in \V(X)$, we measure
...
\begin{defn}\label{def:inc}
    ...
    \[
	\OInc_{\dg M }( \mu) := 
		\!\!\!\!\!\!\sum_{\ed L{X\,}{\,Y} \in \Ar^{\dg M}} \!\!\!\!\!\beta_a^{\dg M} \Ex_{x \sim \mu_{_X}}\!\!
...
% \vfull{
	...
	\[ 
        ...
        \OInc_{\dg M}(\mu) . 
    \]
    is the minimum possible incompatibility of $\dg M$ with any distribution $\mu$.
% }
\end{defn}

\temphide{
The idea behind this definition of inconsistency is to choose a
distribution $\mu$ that minimizes the total number of bits required to
encode all of the relevant conditional marginals. More precisely, fix
a distribution $\mu$. For each arc $\ed aXY \in \Ar$ and $x \in \V X$,
we are given a code for $Y$ optimized for the distribution $\p_a(x)$,
and asked to transmit data from $\mu(Y\mid x)$; we incur a cost for
each bit required beyond what we would have used had we used a code
optimized for the actual distribution $\mu(Y\mid X=x)$. To obtain the
cost for $a$, we take a weighted average of these costs, where the
weight for the value $x$ is the probability $\mu_X(x)$. We do this for
every arc $a \in \Ar$, summing the cost.

For even more intuition, imagine two agents ($A$ and $B$) with
identical beliefs described by a PDG $\dg M$ about a set of variables
that are in fact distributed according to $\mu$. For each arc $\ed aXY
\in \Ar^\dg M$, values $x,y \in \V(X,Y)$ are chosen according to
$\mu(X,Y)$ and $x$ is given to both agents. 

At this point, the agents, having the same conditional beliefs, and
the same information about $Y$, agree on the optimal encoding of the
possible values of $Y$ as sequences of bits, so that if $y$ were drawn
from $\p_a(x)$, the fewest number of bits would be needed to
communicate it in expectation. The value of $y$---which is distributed
not according to $\p_a(x)$, but $\mu(Y \mid X=x)$---is now given to
agent A. The agents pay a cost equal the number of bits needed to
encode $y$ according to the agreed-upon optimal code, but reimbursed
the (smaller) cost that would have been paid, had the agents beliefs
lined up with the true distribution $\mu$.

Repeating for each arc and summing the expectations of these costs, we
can view $\OInc_{\dg M}(\mu)$ as the total number of \emph{additional}
expected bits required to communicate $y$ with a code optimized for
$\p_a(x)$ instead of the true conditional distribution   $\mu(Y \mid
X=x)$. 
...
$\OInc_{\dg M}( \mu) = 0$.  
	...
	% \begin{example}
    %     % [continues=ex:worldsonly]
	%     % Recall our simplest example, which directly encodes an entire distribution $p$
    %     Consider a simple example in which encodes a distribution $p$ over the set $W$. In this case, there is only one arc, the expectation is over
	%     a single element, and the marginal on $W$ is the entire distribution. Therefore,
	%     $\OInc(\dg M; \mu) = \kldiv{\mu}{\mu}$, so the inconsistency is just the
...
We think of each arc $\ed aXY$ as representing a
qualitative claim
...
of $Y$ given the value of $X$ in expectation.
	\label{item:localinfo}
\end{enumerate}
In a sense, (a) is the information needed to determine $\mu$ along its own structure,
while (b) is the information needed to determine $\mu$ along the structure of the hypergraph $\Ar$. 
...
the \emph{information deficiency}
of distribution $\mu$, denoted $\SDef_{\!\Ar}(\mu)$,
is the difference between (a) and (b), 
where we measure the amount of information needed for a description
using entropy: 
\begin{equation}
	\SDef_{G}(\mu) := \sum_{(X,Y) \in \Ar} \H_\mu(Y\mid X) - \H(\mu). 
	\label{eqn:alt-extra}
\end{equation}
...
We illustrate $\SDef_{\dg M}$ with some simple examples.  
Suppose that $\dg M$ has two nodes, $X$ and $Y$.  If $\dg M$ has no arcs, the
$\SDef_{\dg M}(\mu) = - \H(\mu)$. There is no information required to specify, for
each arc in ${\dg M}$ from $X$ to $Y$, the value ${\mat w}_Y$ given ${\mat
w}_X$, since there are no arcs. Since we view smaller numbers as representing a
better fit, $\SDef_{\dg M}$ in this case will prefer the distribution that
maximizes entropy. If $\dg M$ has one arc from $X$ to $Y$, then since $\H(\mu) =
\H_{\mu}(Y \mid X) + \H_\mu(X)$
by the well known \emph{entropy chain rule} \cite{mackay2003information},
$\SDef_{\dg   M}(\mu) = -\H_{\mu}(X)$. Intuitively,
...
on $X$. 
If $\dg M$ has sufficiently many parallel arcs
 from $X$ to $Y$
 and $\H_{\mu}(Y \mid X) > 0$ 
(so that $Y$ is not totally determined by $X$)
then we have $\SDef_{\dg M}(\mu) > 0$, because the redundant arcs add no
information, but there is still a cost to specifying them.
In this case, $\SDef_{\dg M}$ prefers distributions that make $Y$ a
deterministic function of $X$ will maximizing the entropy of the
marginal on $X$.
Finally, if ${\dg M}$ has an arc from $X$ to $Y$ and another from $Y$
to $X$, then a distribution $\mu$ minimizes $\SDef_{\dg M}$ when 
...
In \cref{chap:relent-soup}, we will see how PDGs also naturally arise from another perspective, where they have a semantics in terms of belief updates, so that we can interpret a PDG $\dg M$ as a function 
...
and for an arc $\ed aZY$, take $X_{a} := \{Z,Y\}$, and $\phi_a(z,y) := \p_a^{\dg M}(y \mid z)$ 
% (i.e., $(\p_a^{\dg M}(z))(y)$)
\unskip.
\end{defn}
...
$\Phi$.%
\onlyfirsttime{\footnote{Recall that we identify the unweighted PDG $(\Gr,\mat
...
the fact that the translation from factor graphs to PDGs preserves 
    semantics only for $\gamma=1$ poses no problem:
...
graphical models \cite{wainwright2008graphical}.
% new
...
essentially necessary to get \Cref{theorem:pdg-is-fg,theorem:fg-is-pdg}.
...
Consider the PDG $\dg M$ containing just $X$ and $1$, and two arcs
$p, q: 1 \to X$.
...
They have a of advantages over other
...
A variational autoencoder \cite{kingma2013autoencoding}, for instance, 
is essentially three cpds: a prior $p(Z)$, a decoder $p(X \!\mid\! Z)$, and 
an encoder $q(Z \!\mid\! X)$. Because two cpds target $Z$ (and the cpds are 
inconsistent until fully trained), this situation
