...
with the 
depicted graphically on the right of \Cref{fig:grok-combine}.
%
...
Yet the sum of PDGs performs no such merge.
...
$.5 \balpha_1 + .5 \balpha_2$, where $\balpha_1$ and $\balpha_2$ represent the causal graphs $G_1$ and $G_2$, respectively. 
For instance, if $G_1$ and $G_2$ are distinct causal graphs that represent the same independencies, then $\balpha$ will encode those shared independencies. 
...
% Specifically, this is the case when both the mechanisms 
...
This is often appropriate if the mechanisms are describing completely different things (e.g., if the targets of the two hypergraphs are entirely distinct).
