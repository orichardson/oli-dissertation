...
$\bf 1$ is the constant function (i.e., so that $\alpha_a = \beta_a = 1$ for all $L$). 
...
\begin{defn}\label{def:inc}
    ...
    \[
	\OInc_{\dg M }( \mu) := 
		\!\!\!\!\!\!\sum_{\Ar L{X\,}{\,Y} \in \Ar^{\dg M}} \!\!\!\!\!\beta_a^{\dg M} \Ex_{x \sim \mu_{_X}}\!\!
...
encode all of the relevant conditional marginals. More precisely, fix a
    distribution $\mu$. For each arc $\ed aXY \in \Ar$ and $x \in
    \V X$, we are given a code for $Y$ optimized for the distribution $\p_a(x)$,
    ...
    probability $\mu_X(x)$. We do this for every arc $a \in \Ar$, summing the
    cost.
    ...
    distributed according to $\mu$. For each arc $\ed aXY \in \Ar^\dg
    M$, values $x,y \in \V(X,Y)$ are chosen according to $\mu(X,Y)$ and $x$ is
    ...
    of $Y$ as sequences of bits, so that if $y$ were drawn from $\p_a(x)$, the
    fewest number of bits would be needed to communicate it in expectation. The
    value of $y$---which is distributed not according to $\p_a(x)$, but $\mu(Y
    ...
    Repeating for each arc and summing the expectations of these costs, we can view
    $\OInc_{\dg M}(\mu)$ as the total number of \emph{additional} expected
    bits required to communicate $y$ with a code optimized for
    $\p_a(x)$ instead of the true conditional distribution   $\mu(Y \mid X=x)$. 
    ...
    consistent, then any distribution $\mu \in \SD{\dg M}$ will have $\OInc_{\dg
    M}( \mu) = 0$.  
...
separately specify, for each arc $\ed aXY$, the value $\omega[Y]$ (of $Y$ in world $\omega$) given the value $\omega[X]$, in expectation.
...
of distribution $\mu$, denoted $\IDef_{\!\Ar}(\mu)$,
...
For a PDG ${\dg M}$, we take $\IDef_{\dg M} = \IDef_{\Ar^{\dg M}}$.   
\end{defn}
...
then we have $\IDef_{\dg M}(\mu) > 0$, because the redundant arcs add no
...
$\OInc_{\dg M}(\mu)$ and $\IDef_{\dg M}(\mu)$ give us two measures
...
\begin{equation}
	\IDef_{\dg M}(\mu) := \sum_{\ed aXY \in \Ar} \alpha_a \H_\mu(Y\mid X) - \H(\mu). 
...
\item They allow for a clean separation between quantitiatve information (the
	...
	terms $\OInc$ and $\IDef$ in our scoring function.
...
both $\OInc$ and $\IDef$ are bounded below, it must be the case that  
\[
...
Therefore the per-arc terms for both $\IDef_{\dg M}$
