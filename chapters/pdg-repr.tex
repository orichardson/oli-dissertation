...
\begin{tikzpicture}[center base, scale=0.7, AmpRep]
	...
	\begin{tikzpicture}[center base]
		...
		\node[dpadded, fill=white, draw=gray] (true)  at (0,2) {$\pdgunit$};
...
\fi
        \caption[
        A BN without edges, its corresponding PDG, 
...
In this chapter, with the exception of \cref{sec:expfam}, we implicitly take $\alpha = {\bf 1}$
...
all explicitly.
We write $\V(S)$ for the set of possible joint settings of a set
$S$ of variables.
Thus, we write $\V\!\X$ for all settings of the variables in $\N^{\dg M}$; 
    we refer to these complete joint settings as ``worlds''.
...
This limiting semantics semantics has an important property: 
...
\[
\min_{\mu \in \Delta\V\!\X} \Big[ \Inc_{\dg M}( \mu) + \gamma\lb \Big]
~\leq~ \min_{\mu \in \Delta\V\!\X}\bbr{\dg M }_\gamma(\mu) ~\leq~
\min_{\mu \in \Delta\V\!\X} \Big[ \Inc_{\dg M}( \mu) + \gamma\ub
    \Big], \] 
