...
% How should you 
% We have just
In the previous two chapters, we come to understand a great deal about how to reason \emph{about} PDGs, and the connections between their semantics.
...
In \cref{sec:qual-monotone}, we establish a stronger directed notion of 
...
An obvious candidate definition would be to call two hypergraphs $\Ar_1$ and $\Ar_2$ are QIM-equivalent if they are compatible with same distributions ($\mu \models \Diamond\Ar_1$ iff $\mu \models \Diamond \Ar_2$),
...
The result forms the beginnings of an inconsistency-tolerant logic. 
...
The reason why is that implication allows for substitution in within context. For instance, if $\varphi \Rightarrow \psi$, then 
...
Our key contributions in this section are a pair of two reasoning principles that can prove phenomena such as BN equivalence (\cref{theorem:bn-completeness}), and help us begin to make sense of cyclic structures from the opposite direction as studied in \cref{sec:qim-info}. 
