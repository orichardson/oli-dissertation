...
\newmaterial{%
We will begin with a simple but conceptually important observation: 
    a joint probability distribution is a special case of a PDG (\cref{sec:prob-as-pdg}). 
    % Thus, the standard epistemic state of a 
% This means that a standard
% So, because standard 
In \cref{sec:widgets}, we develop a library of smaller tools, which we call \emph{widgets}, that can capture various other important fragments of epistemic information. 
...
In \cref{sec:other-uncertainty-repr}, we will then use many of the tools developed in \cref{sec:widgets} to show that PDGs capture other well-established representations of uncertainty, most notably sets of probability measures, known as credal sets \citep{Walley1991-SRIP} (\cref{ssec:cvx-prob-sets}) and Dempster-Shafer Belief functions \cite{shafer1976mathematical} (\cref{ssec:belplaus-as-pdg}).
% In the remainder of the section, we 
We also quickly overview other representations captured by PDGs that will play larger roles later on: cluster (pseudo)marginals, causal models, and neural networks. 

But first, we start with probability.
...
Still, probability does have shortcomings \cite{}, and there have been many approaches to representing knowledge and uncertainty \cite{halpern2017reasoning}.
...
We start by recounting an observation we already made in \cref{sec:var-randomvar},
...
It is easy to verify that these PDGs have inconsistency zero, and represent precisely the distribution $\mu$, no matter what confidence parameters we select (so long as the confidence is positive). 
