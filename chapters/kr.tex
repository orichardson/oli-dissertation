...
\temphide{
\paragraph{Updating with Combination.}
\TODO
}
...
of values that satisfy the constraint, or equivalently, 
...
\newmaterial{%
    Visually, all we have done is to shrink the variable $\Tru$ to a single point, and suppress the label $\trut$ on the event $\Tru{=}\trut$, leaving only a pair of double-headed arcs joined head-to-head.
    Just as we have left $\Tru$ implicit in the graphical notation,
}%
% Correspondingly, 
we will typically identify a distribution $\mu(\X)$ with 
...
\subsection{Costs and ``Soft'' Constraints}
...
There is already an obvious candidate for how to do this: simply do precisely what we did for a hard constraint, except use a cpd $c(\Tru \mid X)$ that is not deterministic
    in place of $\delta R(\Tru\mid X)$. 

As before add the variable $\Tru$ that takes values $\{\trut, \truf\}$, and the event $\Tru \!\!=\! \trut$, to any PDG.
% Now, given a
...
\begin{linked}{prop}{log-barier}
    Given a PDG $\dg M$ and a function $s : \V\!X \to [0,1]$. 
    %
...
Couplings are an important tool for probabilistic reasoning, and are the basis of optimal transport theory \citep{optimal-transport}, 
...
% As discussed in \cref{sec:bns}, \cref{theorem:bns-are-pdgs} is a powerful example of how 
In \cref{ex:coin}, the PDG representation lead us to a space which, upon applying the principle of maximum entropy (or better yet, of minimum information deficiency) leads to a more reasonable answer than the naive space given by the set of possible marginals over the coin outcome. 
% It is worth noting that the naive space is not even 
It is difficult to imagine how one could even obtain the naive maximum entropy result with a PDG.
}


In the next section, we show that a more specific representation of uncertainty
    can be captured with PDGs in a more targeted way.
