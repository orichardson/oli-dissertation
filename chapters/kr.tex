...
\begin{center}
    \begin{tikzpicture}[xscale=1.6]
        ...
        \foreach \i in {1,...,5} {
            ...
            \draw[arr2, shorten <=0pt] (mergeorigin) to (X\i);
        }
...
\begin{prop}
    ...
    \begin{enumerate}[topsep=0pt]
        \item $\SD{R\,} 
            = \{ \mu(\mat X)\delta(\Tru{=}\trut) : \Supp \mu \subseteq R \}
            = \{ \mu(\mat X)\delta(\Tru{=}\trut) : \mu(R) = 1 \}
            $.
        \item For all PDGs $\dg M$
            % containing the variables $\mat X$:
            with variables $\X \supseteq \mat X$, 
        \begin{enumerate}
            \item $\SD{\dg M + R} = \SD{\dg M} \cap
                % \{ \mu(\X) : \Supp \mu \subseteq R\}
                \{ \mu(\X) : \mu(R) = 1\}.
                % \SD{R}
                $
            \item for all $\gamma \ge 0$, $\aar{\dg M + R}_\gamma \ge \aar{\dg M}_\gamma$ with equality iff $\mu(R) = 1$ for all $\mu \in \bbr{\dg M}_\gamma$. 
            
            \item provided $\aar{\dg M} < \infty$, $\bbr{\dg M + R}^*$ consists entirely of distributions $\mu$ satisfying $\mu(R) = 1$. 
        \end{enumerate}
    \end{enumerate}
\end{prop}
\begin{proof}
...
% $\dg M^{y|x=p}:=$
    ...
    \begin{tikzpicture}[center base]
        ...
        \node[Dom={%
            %$\XxYy$%
            $\XxYyshort$%
                [label distance=-1.5em, xshift=3.2em] (XxYy)
...
Now, suppose we have a distance metric $d$ on a space $X$.
For $k \in [1,\infty)$, 
