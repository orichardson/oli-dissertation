...
\newmaterial{%
% It is easier to argue that there is 
It is easier to think critically about beliefs than values.
% In general, it is hard to argue against values at all.
While beliefs can be inspected and tested by reality, it is difficult in general to take a principled stand in favor of a value.
But to want to hold consistent beliefs is a uniquely principled kind of value,
...
Moreover, much of the current practice of building AI systems can be done with this value alone. 
    % representing inconsistent beliefs. 
% By representing inconsistent beliefs, 
% But 
% And consistency (of beliefs) is a value that is both supported by a rich : there is a rich history of arguments against inconsistency bad. 
% Moreover, it seems li
}%
Perhaps our AIs do not need to take a mathematically undefended stand in the space of values at all. 
Maybe, if we are careful, we may be able to lean mostly on a single \emph{universal} value: 
% \begin{center}
% \\\hfill
