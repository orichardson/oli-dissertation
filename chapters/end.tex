...
\else
    ...
    This observation raises dozens of interesting questions about the category theory of PDGs, which we are only just beginning to explore.
    ...
    We suspect such a connection may exist; after all, there are important categorical characterizations of relative entropy    \citep{baez2014bayesiancharacterizationrelativeentropy} 
        and of information loss \citep[Theorem 12.4.9]{leinster2021entropy} in these terms. 
...
\newmaterial{%
    ...
    the same is not true of values in general. 
}%
...
\newmaterial{%
Moreover, as shown in \cref{part:univ-objective}, much of the current practice of artificial intelligence can be done by articulating beliefs (which are subject to analysis and revision) and appealing to this one particularly special value.
% Moreover, this value alone (together with much of the current practice of artificial intelligence can be done by appeal to this value alone.
}%
% By representing inconsistent beliefs, 
So perhaps our AIs do not need to take a mathematically undefended stand in the space of values at all. 
Maybe, if we are careful, we may be able to lean 
    solely on (possibly inconsistent)
    beliefs and a single \emph{universal} value: 
% \begin{center}
% \\\hfill
