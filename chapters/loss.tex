...
\begin{equation}
    \Inc_{\dg M}(\mu)\! := \!\!\!\sum_{\ed aXY}\!\! \beta\ssub a  \Ex_{{x \sim \mu(\mskip-2.6muX\mskip-2mu)}} \kldiv[\Big]{\mu(Y|\, x)}{p_a(Y |\, x)}.
    \label{eq:inc}
\end{equation}
% \vskip-2ex
...
is the unique one that minimizes both $\Inc_{\dg B}$ and $\SDef_{\dg B}$ (and
...
sum $\Inc_{\Phi} + \SDef_{\Phi}$.
In general, for any $\gamma > 0$, one can consider a weighted combination
\(
    \bbr{\dg M}_\gamma(\mu) \!:= \Inc_{\dg M}(\mu) + \gamma\; \SDef_{\dg M}(\mu),
\)
