...
\begin{center}
    \small\singlespacing
    Adapted and exteded from \textit{Loss as the Inconcsistency of a Probabilistic Dependency Graph: Choose Your Model, not Your Loss Function}, originally 
    published in AISTATS 2022 \cite{one-true-loss}. 
\end{center}
...
A choice of $\beta\ssub a=0$ means that the cpd $\p_a$ is effectively ignored, in the sense that such a PDG is equivalent to one in which the edge is attached to a different cpd $q \ne \p_a$.
On the other hand, a large value of $\beta\ssub a$ (or $\infty$) indicates high (or absolute) confidence in the cpd. By default, we suppose $\beta\!=\! 1$, which is just a convenient choice of units---what’s important are the magnitudes of $\beta$ relative to one another.
...
\begin{equation}
    \OInc_{\dg M}(\mu)\! := \!\!\!\sum_{\ed aXY}\!\! \beta\ssub a  \Ex_{{x \sim \mu(\mskip-2.6muX\mskip-2mu)}} \kldiv[\Big]{\mu(Y|\, x)}{p_a(Y |\, x)}.
    \label{eq:inc}
\end{equation}
% \vskip-2ex
...
So $\OInc_{\dg M}(\mu)$ measures the total excess cost of
...
$\aar{\dg M} := \inf_\mu \OInc_{\dg M}(\mu)$.
...
% \vskip-3ex
\[
	\SDef_{\dg M}(\mu) := -\H(\mu) + \sum_{\ed aXY} \alpha\ssub a \H_\mu(Y\mid X).
\]
% \vskip-2ex
$\SDef_{\dg M}(\mu)$ %
can be thought of as the
information
needed to separately describe the target of each edge $a$ given the value of its source
(weighted by $\alpha\ssub a$)
...
\Cref{prop:expected-surprise,prop:marginal-ll} extend the surprisal result (\cref{prop:pdg-Ix})
    in two orthoganal directions: to settings with multiple observations, and with partial observation.
...
\[
    ...
    \bigg\}\!;
\]
See figure \cref{fig:entropy-cube} for a full illustration.
...
\begin{figure}
	...
	\caption[%
        Statistical distances as inconsistencies: amap of the inconsistency of $p(X)$ and $q(X)$, as their respective confidences vary.]
...
\end{prop}
% \vskip-1.3ex
This result gives us an intuitive interpretation of the asymmetry of relative entropy (i.e., KL divergence), and a prescription for when it makes sense to use it.
	   ...
	   &=\! \Ex_{x\sim D}\Bigg[
			\inf_{\mu(Y)} \Ex_{y \sim \mu}
				\left[ (\beta_1 \!+\! \beta_2) \log \frac
				 ...
				 + \xi(x)
		   \Bigg] 
	\intertext{but now the entire first term is the infemum
        of a relative entropy, which is non-negative and equal to zero iff $\mu(y) = \mathcal N(y | g(x),\tilde\sigma(x)\sqrt{\beta_1\!+\!\beta_2})$. So the infemum on the left is equal to zero.
	}
