...
% \forjoe{This text was here in the original paper. I stand by surprise being a kind of epistemic conflict. I polled a few friends and language models (``is surprise a kind of epistemic conflict, or not?''), and uniformly got positive answers. 
% I've given several talks where I use this line, and have seen a lot of nods.
...
% \forjoe{%
% 	cross entropy and relative entropy are different quantities, but related, and equivalent for optimization.  
% 	I think this is well known.  In any case, we have a whole bolded paragraph discussing this in the preliminaries, and it also becomes clear if you compare this result with Prop 6.9.
% }%
...
% \forjoe{There's no dash in ``factor graph'', as standardly written.}
% \forjoe{You say the dash is important for grouping, but there is no well-accepted right association rule for noun phrases. When you say ``plastic dog food bowl cover'', you do mean a ``plastic (((dog food) bowl) cover)'', not a ``plastic (dog (food (bowl cover)))''. Dashes can be useful for aiding ambiguity, especially with adjectives (i.e., single-family homes instead of single family homes), but cannot fully resolve the ambiguity in general. I do not feel a dash here resolves any ambiguity, and I have never seen ``factor-graph'' written with a dash even when part of a larger noun phrase. 
%  }
...
The answer,
as we discovered in \cref{sec:soft-constraint-widget},
...
$
	\hat c(\trut | x) := e^{-c(x)}.
$
% By threatening to generate the 
By investing belief in the
falsehood {\truf} with probability dependent on the cost of $X$, the cpd $\hat c$ links the value of $X$ directly to an inconsistency. 
\begin{linked}{prop}{expected-cost}
	\!
	\( \displaystyle
		\aar*{\!\begin{tikzpicture}[center base]
			\node[dpad0] (X) at (0,0) {$X$};
			\node[dpad0] (2) at (1.3,0) {$\Tru$};
...
but doing so is necessary to recover an inconsistency equal to $\Ex_p [c]$.%
...
\newmaterial{%
We return to this point when we conclude in \cref{chap:end}. 
}%
...
\begin{lproof}\label{proof:marginal-ll}
	...
	\aar*{
		...
		\end{tikzpicture}}
			...
			&= \log \frac{1}{p(x)} = \I_p(x) & \hspace{-2cm}\text{[Gibbs Inequality]}
	\end{align*}
\end{lproof}
...
\begin{lproof}\label{proof:accuracy}
	...
	\begin{align*}
		...
		&\backtab= \beta \Ex_{ D \mid f=h} \log \frac{1}{D(f\!=\!h)}
			& \hspace{-2em}{\singlespacingmath \left[ \begin{tabular}{c}
	...
	\begin{align*}
        ...
        &=  \beta \kldiv{\mu(X)}{p(X)} +
			...
			&= \OInc_{\dg M_1}(\mu(X)) + (\beta\!+\!\zeta) \Ex_{x \sim \mu}\kldiv{\mu(Y|x)}{f(Y|x)}
				\\
			&= \aar{\dg M_1}
				& \hspace{-4in}{\singlespacingmath \Big[\begin{array}{c}
