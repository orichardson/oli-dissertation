...
At the opposite extreme, if $\chi$ is infinite in every component,
    then the final line
...
In \cref{chap:one-true-loss}, we found that standard loss functions can be viewed as the inconsistency of a PDG.
...
When the focus is on generation, we have control over $G$.
...
Each message is an (unnormalized) distribution over the corresponding variable ($X$).
...
\end{figure}
    ...
    \[
        \varphi_j(a) :=  {\singlespacingplusmath \begin{cases}
...
What's missing is a way to encode the idea 
...
It also illustrates how,
    in the process of resolving inconsistency,
    restricting control to a small set of parameters can be 
        appropriate (and not just a computational shortcut).
