...
\section{LIR in Machine Learning}
...
\subsection{Generative Adversarial Training as LIR}
...
More precisely, 
\oldnewmaterial{%
represent the generator network as a distribution $G(X_\fake)$  over fake images,
...
This state of affairs is summarized as the PPDG below, in which $\Theta$ consists of joint settings of the parameters of the two networks ($G$ and $D$). 
...
After initialization, belief propagation repeatedly recomputes:
% The bulk of the algorithm involves propagating messages, each of which
...
The traditional depiction of messages
    ...
    we obtain an equivalent PDG that does not look so different: 
    \begin{center}
    \begin{tikzpicture}[center base,
...
In this section, we show how standard decision rules can be viewed as inconsistency minimization. 

\subsection{Expected Utility Maximization as LIR}
...
Let $\dg M_{(p, \tau, u, b)}$ represent the PDG above, augmented with such a soft constraint $b$. 

Intuitively, imagine that there is a part of you that you cannot control, 
    corresponding to the sub-PDG made up of $u$, $b$, and $\Tru{=}\trut$,
    which we will call ``Faith''.
Faith engages in wishful thinking: 
    she disbelieves outcomes that are undesirable,
    creating epistemic conflict if she sees outcomes of low utility. 
If you have no control over Faith, but still find her worth paying attention to, selecting actions to minimize inconsistency is the same as selecting actions to maximize expected utility. 
...
\begin{prop}
    ...
    $\aar{\dg M_{(p,\tau,u,b)} + A{=}a}_\gamma$ are those that maximize expected utility.
    In other words,
    for all $\gamma \ge 0$, 
    \[
