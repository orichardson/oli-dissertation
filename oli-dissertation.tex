...
To be \emph{rational}, you immediately resolve the inconsistency, so as to maintain a consistent (probabilistic) picture of the world.
%
...
Inconsistency is clearly undesirable, but we stand to gain a lot by being able to represent it.
%
...
This thesis develops a broad theory of how to approach probabilistic modeling with possibly-inconsistent information.  
The key ingredient is a novel kind of graphical model, called a \emph{Probabilistic Dependency Graph (PDG)}. 
PDGs allow a modeler to specify arbitrary (even conflicting) pieces of probabilistic information with arbitrary degrees of confidence. 
...
In \cref{part:univ-objective}, we show that PDGs also capture modern neural representations. Indeed, it turns out that most loss functions in the literature can be viewed as the inconsistency of a PDG that models the situation appropriately. Furthermore, many of the most important algorithms in AI can be viewed as instances of a simple heuristic approach to resolving inconsistencies. 
In \cref{part:algo-logic-complexity}, we formalize and provide algorithms for inference in PDGs, and investigate the deep algorithmic connection between inference in a PDG, and calculating a PDG's numerical degree of inconsistency. We also develop general principles for reasoning with (and about) PDGs. 
\end{abstract}
